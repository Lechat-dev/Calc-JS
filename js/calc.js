class Calc {
  ...
  static compute(arr) {
    const strExpression = arr.map(x =>
      /[-+*\/\.\d]/g.test(x) ? x.match(/[-+*\/\.\d]/g).join('') : x,
    );
    const numExpression = strExpression.map(x =>
      parseFloat(x) ? parseFloat(x) : x,
    );
...
refs.form.addEventListener('click', e => {
  ...
  if (dataID === 'maths' && refs.canMath) {
    if (refs.operand.endsWith('.')) {
      ...
      !refs.operand &&
      refs.expression[last] &&
      refs.expression[last].endsWith(' ')
    ) {
      refs.expression.pop();
      refs.expression.push(` ${symbol} `);
      refs.canDot = true;
